...
import { SAMPLE_DATA } from "../lib/sample_data.js";
...
const emptyPreview = `<div style="padding:24px;text-align:center;color:var(--text-mute);font-size:11.5px;height:100%;display:grid;place-items:center">
  Sin contenido todavía.<br>Editá esta planilla para agregar HTML y CSS.
</div>`;

const renderMiniPreview = async (codigo, container) => {
  try {
    const pair = await resolvePlanilla(codigo);
    if (pair.empty) { container.innerHTML = emptyPreview; return; }
